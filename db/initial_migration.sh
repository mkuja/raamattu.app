...
NEEDS_MIGRATION=$(psql -h $PG_HOST -U postgres -d "$POSTGRES_DB" -t -c "SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name='books');")

if [[ "$NEEDS_MIGRATION" =~ "f" ]]; then
    echo "Setting up initial database from dump."
    tar -xJvf raamattu.pgsql.tar.xz
    psql -h $PG_HOST -U postgres -d "$POSTGRES_DB" -f raamattu.pgsql
